...
/**
 * fetch-scrutinio.js
 * Scarica i risultati storicizzati delle elezioni comunali 2021
 * di San Benedetto del Tronto dal portale Eleweb.
 *
 * Struttura URL Eleweb – dati storicizzati:
 *   Snapshot:  /static_json/storico/folder.js
 *                → contiene: folderToRead = 'YYYYMMDD_HHMMSS'
 *
 *   Totali (sezione 0):
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_raggrup_{PARTIZIONE}.json
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_liste_{PARTIZIONE}.json
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_candi_{PARTIZIONE}.json
 *
 *   Per sezione N:
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_raggrup_{PARTIZIONE}_{N}.json
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_liste_{PARTIZIONE}_{N}.json
 *     /static_json/storico/{SNAPSHOT}/{CDELE_NUMERIC}/voti_candi_{PARTIZIONE}_{N}.json
 *
 * Elezione: AMMINISTRATIVE 2021 - DATI VALIDATI ALL'UFFICIO CENTRALE
 *   cdele=3  macro=S3  path=storico
 */
...
const CDELE_NUMERIC  = '3';    // corrisponde a macro_cdele "S3" (validati 2021)
const PARTIZIONE     = '0';    // idpartizione (SBT usa 0)
const N_SEZIONI      = 50;     // SBT 2021: 43 sezioni — Eleweb restituisce 404 per quelle inesistenti
const DATA_DIR       = path.join(__dirname, 'data');

// URL da cui estrarre il timestamp snapshot storico
const FOLDER_JS_URL  = `${BASE_SITE}/static_json/storico/folder.js`;
...
async function resolveSnapshotPath() {
  console.log('Resolving storico snapshot from folder.js...');
  const js = await fetchText(`${FOLDER_JS_URL}?_=${Date.now()}`);
  const match = js.match(/folderToRead\s*=\s*['"](\d{8}_\d{6})['"]/);
  if (!match) throw new Error(`Snapshot non trovato in storico/folder.js. Contenuto: ${js.slice(0, 200)}`);
...
function buildUrl(snapshot, tipo, sez) {
  ...
  return `${BASE_SITE}/static_json/storico/${snapshot}/${CDELE_NUMERIC}/${suffix}`;
}
...
async function main() {
  ...
  console.log(`\n=== fetch-scrutinio.js 2021  ${fetchedAt} ===\n`);
  ...
  const meta = {
    ...
    snapshot,
    cdele:              CDELE_NUMERIC,
    descrizione:        'AMMINISTRATIVE 2021 - DATI VALIDATI',
